...
int run_preemptive_first_round(P p_array[], int len) {

    int space, finished = 0;
    ...
    for (int i = 0; i < len; i++) {
        ...
        /* process can't run more than it's remaining time */
        p_array[i].Burst_Time = min(p_array[i].Burst_Time, p_array[i].Remaining_Time);

        simulate_cpu_burst(&p_array[i]); /* simulate a burst */
        time += p_array[i].Burst_Time;

        if (i + 1 < len && p_array[i].Remaining_Time) {

            space = p_array[i + 1].Arrival_Time - time;
            while (space > 0) {
